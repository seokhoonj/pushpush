...
def test_send_text_prints_provider_and_exits_zero(
    config_dir, transport, monkeypatch, capsys
):
...
def test_send_reads_text_from_stdin_when_absent(
    config_dir, transport, monkeypatch
):
    _telegram_route(config_dir, monkeypatch)
...
def test_send_passes_flags_through(config_dir, transport, monkeypatch):
    _telegram_route(config_dir, monkeypatch)
...
def test_a_failed_send_reports_and_exits_one(config_dir, transport, capsys):
    write_config(
...
def test_a_network_failure_exits_two(config_dir, transport, monkeypatch, capsys):
    _telegram_route(config_dir, monkeypatch)
...
def test_send_media_through_the_cli(config_dir, transport, monkeypatch, tmp_path):
    _telegram_route(config_dir, monkeypatch)
...
def test_an_argument_wins_over_piped_stdin(config_dir, transport, monkeypatch):
    _telegram_route(config_dir, monkeypatch)
...
def test_no_text_and_a_tty_exits_one(config_dir, transport, monkeypatch):
    _telegram_route(config_dir, monkeypatch)
