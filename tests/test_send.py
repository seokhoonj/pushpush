...
def _telegram_route(config_dir, monkeypatch):
    write_config(
        config_dir,
        '[routes.alerts]\nprovider = "telegram"\ndestination = "123"\n',
    )
...
def test_send_returns_receipt(config_dir, transport, monkeypatch):
    _telegram_route(config_dir, monkeypatch)
...
def test_send_uses_default_route(config_dir, transport, monkeypatch):
    _telegram_route(config_dir, monkeypatch)
    receipt = send("hello")  # no `to`; single route is the default
    assert receipt.route == "alerts"


def test_send_reaches_the_provider(config_dir, transport, monkeypatch):
    _telegram_route(config_dir, monkeypatch)
...
def test_send_media_goes_multipart(config_dir, transport, monkeypatch, tmp_path):
    _telegram_route(config_dir, monkeypatch)
...
def test_send_without_secret_is_reported(config_dir, transport):
    write_config(
        config_dir, '[routes.alerts]\nprovider = "telegram"\ndestination = "1"\n'
    )
    with pytest.raises(MissingSecretError):
        send("hello", to="alerts")


def test_send_receipt_response_is_read_only(config_dir, transport, monkeypatch):
    _telegram_route(config_dir, monkeypatch)
    receipt = send("hello", to="alerts")
...
def test_media_on_slack_route_is_refused_before_send(
    config_dir, transport, monkeypatch, tmp_path
):
    write_config(config_dir, '[routes.team]\nprovider = "slack"\n')
...
def test_send_accepts_an_explicit_config(config_dir, transport, monkeypatch):
    _telegram_route(config_dir, monkeypatch)
    config = load_config()
